...
public class ARUIManager : MonoBehaviour
{
    ...
    private void OnClickedMoreInfoMap(String icon)
    {
        ...
        LeanTween.scale(infoBox, new Vector3(0.1719052f, 0.1409813f, 0.31823f), 0.3f);
        distanceBox.SetActive(false);
    ...
    {
        distanceBox.SetActive(false);
        LeanTween.scale(infoBox, new Vector3(0, 0, 0), 0.3f).setOnComplete(DeactivateInfoBox);
    }
    ...
    {
        muralLocations.Add("icon1", "Mural: The Last King of Ireland, \n155 meters away [S]");
        muralLocations.Add("icon2", "Mural: Brigid of Faughart, \n245 meters away [N]");
        muralLocations.Add("icon3", "Mural: Ní neart go cur le chéile,\n648 meters away [N]");
    }

    private void SetMuralInfo()
    {
        muralInfo.Add("icon1", "ARTIST - achesdub \nMURAL: The Last King of Ireland");
        muralInfo.Add("icon2", "ARTIST - Friz \nMURAL: Brigid of Faughart");
        muralInfo.Add("icon3", "ARTIST - Chula Mente \nMURAL: Ní neart go cur le chéile");
    }

    private void SetMuralImage(String icon)
    {
        if (icon == "icon1")
        {
            imagePanel.GetComponent<Image>().sprite = muralImages[0];
        }
        else if (icon == "icon2")
        {
            imagePanel.GetComponent<Image>().sprite = muralImages[1];
        }
        else if (icon == "icon3")
        {
            imagePanel.GetComponent<Image>().sprite = muralImages[2];
        }
    }
}